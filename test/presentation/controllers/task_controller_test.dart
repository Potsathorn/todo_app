  ...
  test(
    ...
    final tasksToFetch = [
      ...
      TaskEntity(
          ...
          status: 'IN_PROGRESS'),
      TaskEntity(
          title: 'C', createdDate: DateTime(2023, 12, 1), status: 'COMPLETED'),
    ];

    when(mockTaskUsecase.get(page: anyNamed('page'), limit: anyNamed('limit')))
        .thenAnswer((_) => Future.value(tasksToFetch));
    ...
    expect(taskController.searchQuery, '');
    expect(taskController.selectedID, '');
    expect(taskController.tasks.length, 3);
    expect(
        sortedTasks,
        containsAllInOrder([
          DateTime(2023, 10, 1),
